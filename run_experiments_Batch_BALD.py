...
def bald_acquire(train_pairs, pool_pairs, candidate_indices):
    j = 0
    for i in candidate_indices:
        train_pairs.append(pool_pairs.pop(i-j))
        j = j+1
...
def run(cut, cut_mode):
    ...
    if OPTIONS['active']:
        ...
        while True:
            ...
            f, cov = model.predict_f(out_feats=train_feats)

            logits_N_K_C = []
            f_samples = []
            for i in range(len(f)):
                f_sample = multivariate_normal.rvs(mean=f[i], cov=cov[i], size=num_inference_samples)
            ...
            for i in range(len(v)):
                v[i] = train_idxs.index(v[i])
                u[i] = train_idxs.index(u[i])

            for every_f in f_samples:
                one_probs = pref_likelihood(fmean=every_f , v=v, u=u)
                zero_probs =np.array([1-p for p in one_probs])
                probs = list(zip(one_probs,zero_probs))
                logits_N_K_C.append(probs)
            logits_N_K_C = np.array(logits_N_K_C)
            logits_N_K_C = logits_N_K_C.transpose((1,0,2,3))
            shape1,shape2,shape3,shape4 = logits_N_K_C.shape

            logits_N_K_C = logits_N_K_C.reshape(shape1,shape2,shape3)
            logits_N_K_C = torch.from_numpy(logits_N_K_C).log()

            bald_acquire(train_pairs, pool_pairs, get_batchbald_batch(
                    logits_N_K_C, batch_size, 10000))
...
if __name__ == '__main__':
    ...
    for rnd_seed, np_seed in seeds:
        ...
        experiment_dir = 'experiment_{}_{}_batchbald_500_10_1000'.format(task, datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
