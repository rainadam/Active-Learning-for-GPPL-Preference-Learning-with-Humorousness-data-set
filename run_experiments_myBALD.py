...
def split_data(pairs, idx_instance_list, train_size=0.6, dev_size=0.2, test_size=0.2, cut=None, cut_mode=None):
    ...
    if cut is not None:
        ...
        with open(os.path.join(OPTIONS['experiment_dir'], 'items_{}_{}.txt'.format(str(cut), cut_mode)), 'w',
                  encoding='utf-8') as f:
            ...
            for pair in cut_train_pairs:
                ...
                writer.writerow(row)
        with open(os.path.join(OPTIONS['experiment_dir'], 'items_instances_{}_{}.txt'.format(str(cut), cut_mode)), 'w',
                  encoding='utf-8') as f:
    ...
    with open(path, 'r', encoding='utf-8') as f:
        ...
        for line_no, line in enumerate(reader):
            for i in range(0, 3):  # 3 comparisons per HIT/line
                ...
                bws_vuamc_ids = OrderedDict(
                    zip(line[27 + (i * 16):27 + (i * 16) + 4], line[27 + (i * 16) + 4:27 + (i * 16) + 4 + 4]))
                con_id = bws_vuamc_ids[line[87 + (i * 2)]] if line[87 + (
                            i * 2)] in bws_vuamc_ids else None  # most conventionalized (i.e. "worst")
                nov_id = bws_vuamc_ids[line[88 + (i * 2)]] if line[88 + (
                            i * 2)] in bws_vuamc_ids else None  # most novel (i.e. "best")
                # check hit validity, otherwise skip
                if nov_id is None or con_id is None or None in [re.match(r'^\d+\.\d+\.\d+$', vuamc_id) for vuamc_id in
                                                                bws_vuamc_ids.values()]:
                    continue
                ...
                # otherwise we use all induced pairs, e.g. the "best" instance is better than all others, ...
                    ...
                    for current_id in bws_vuamc_ids.values():
                        if current_id != nov_id:  # best (a) > b, c, d
                            pairs.append((idx_instance_list.index(nov_id),
                                          idx_instance_list.index(current_id)))  # append "novel" pairs
                            if current_id != con_id:  # worst (d) < b, c
                                pairs.append((idx_instance_list.index(current_id),
                                              idx_instance_list.index(con_id)))  # append "conventionalized" pairs
    return pairs, idx_instance_list
...
def extract_features(requested_idxs, idx_instance_list, embeddings, add_feats):
    ...
    emb_dim = 2 * emb_dim if OPTIONS[
                                 'task'] == 'metaphor' and metaphor_mode == 'concat' else emb_dim  # size of the concatenated embeddings without additional features
...
def train(training_split, train_idxs, idx_instance_list, embeddings):
    ...
    model = GPPrefLearning(ninput_features=items_feat.shape[1],
                           kernel_func=OPTIONS['kernelfunc'],
                           # 'matern_3_2' is default, 'diagonal' is used for testing without features
    ...
    print('a1_train', np.array(a1_train).shape, 'a2_train', np.array(a2_train).shape, 'prefs_train',
          np.array(prefs_train).shape)
    model.fit(np.array(a1_train), np.array(a2_train), items_feat, np.array(prefs_train, dtype=float),
              optimize=OPTIONS['optimization'], input_type='binary')
...
def random_acquire(train_pairs, pool_pairs, batch_size):
    for i in range(batch_size):
        id = random.randint(0, len(pool_pairs) - 1)
        train_pairs.append(pool_pairs.pop(id))


def bald_acquire(train_pairs, pool_pairs, batch_size, IG, train_idxs):

    pool_pairs_IG = []
    for pair in pool_pairs:
        pool_pairs_IG.append(IG[train_idxs.index(pair[0]), train_idxs.index(pair[1])])
    pool_pairs_and_IG = list(zip(pool_pairs, pool_pairs_IG))
    pool_pairs_and_IG = sorted(pool_pairs_and_IG, key=itemgetter(1), reverse=True)
    pool_pairs, _ = zip(*pool_pairs_and_IG)
    pool_pairs = list(pool_pairs)

    added_pairs = dict()

    i = 0
    j = -1
    while i < 0.3*batch_size:
        j = j + 1
        if j >= len(pool_pairs):
            while i < batch_size:
                i = i+1
                j = random.randint(0, len(pool_pairs)-1)
                train_pairs.append(pool_pairs.pop(j))
            break
        pair = pool_pairs[j]
        if pair[0] in added_pairs.keys() and pair[1] in added_pairs.keys():
            if added_pairs[pair[0]] < 3 or added_pairs[pair[1]] < 3:
                train_pairs.append(pair)
                pool_pairs.pop(j)
                ...
                added_pairs[pair[0]] = added_pairs[pair[0]] + 1
                added_pairs[pair[1]] = added_pairs[pair[1]] + 1
        else:
            train_pairs.append(pair)
            pool_pairs.pop(j)
            ...
            if pair[0] in added_pairs.keys():
                added_pairs[pair[0]] = added_pairs[pair[0]] + 1
                added_pairs[pair[1]] = 1
            elif pair[1] in added_pairs.keys():
                added_pairs[pair[1]] = added_pairs[pair[1]] + 1
                added_pairs[pair[0]] = 1
            else:
                added_pairs[pair[0]] = 1
                added_pairs[pair[1]] = 1
    ...
    while i < batch_size:
        ...
        if pair[0] in added_pairs.keys() and pair[1] in added_pairs.keys():
            pass
        elif (pair[0] in added_pairs.keys()) and (added_pairs[pair[0]] < 3):
            train_pairs.append(pair)
            pool_pairs.pop(j)
            added_pairs[pair[0]] = added_pairs[pair[0]] + 1
            added_pairs[pair[1]] = 1
        ...
        elif (pair[1] in added_pairs.keys()) and (added_pairs[pair[1]] < 3):
            train_pairs.append(pair)
            pool_pairs.pop(j)
            added_pairs[pair[1]] = added_pairs[pair[1]] + 1
            added_pairs[pair[0]] = 1
...
def bald_acquire_copy(train_pairs, pool_pairs, batch_size, IG, train_idxs):
...
def compute_pairwise_scores(f, Cov):
    ...
    H_updated = (-np.log(0.5)) * C / np.sqrt(C ** 2 + pairwise_var) * np.exp(
        - (f - f.T) ** 2 / (2 * (C * 2 + pairwise_var)))
...
def run(cut, cut_mode):
    ...
    if OPTIONS['active']:
        ...
        train_idxs = idxs[:int(len(idxs) * train_perc)]
        dev_idxs = idxs[int(len(idxs) * train_perc):-int(len(idxs) * dev_perc)]
        test_idxs = idxs[int(-len(idxs) * test_perc):]

        # only use pairs for training which have both idxs in the train_idxs set
        all_train_pairs = []
        for pair in pairs:
            if pair[0] in train_idxs and pair[1] in train_idxs:
                all_train_pairs.append(pair)
        random.shuffle(all_train_pairs)
        # print("all_train_pairs size", len(all_train_pairs)) 41028

        train_pairs = all_train_pairs[:original]
        pool_pairs = all_train_pairs[original:]

        spearman_correlation = 0
        # get gold_v on dev set
        rows = []
        gold_v = []
        if OPTIONS['task'] == 'metaphor':
            for dev_idx in dev_idxs:
                ...
                rows.append([vuamc_id, metaphor.score, metaphor.covered_text, sentence.pp()])
                if metaphor.score is not None and pred is not None:
                    gold_v.append(float(metaphor.score))
        elif OPTIONS['task'] == 'humour':
            scores = dict()
            with open(HUMOUR_BWS_SCORES_PATH, 'r', encoding='utf-8') as f:
                for line in f:
            ...
            for dev_idx in dev_idxs:
                ...
                gold_v.append(bws)
                rows.append([dev_idx, bws, ' '.join(instance)])
        dev_feats = extract_features(dev_idxs, idx_instance_list, embeddings, add_feats=OPTIONS['add_features'])
        train_feats = extract_features(train_idxs, idx_instance_list, embeddings, add_feats=OPTIONS['add_features'])
        while True:
            print("\n\n\nactive learning start: current size = %d\nspearman_correlation = %f" % (
            len(train_pairs), spearman_correlation))
            print("size of pool pairs is:%d" % len(pool_pairs))
            # train model
            model = train(train_pairs, train_idxs, idx_instance_list, embeddings)
            ...
            with open(os.path.join(OPTIONS['experiment_dir'], output_filename + '.csv'), 'w', encoding='utf-8') as f:
                writer = csv.writer(f)
            ...
            if len(train_pairs) + batch_size > max_samples:
                break;
            if len(pool_pairs) < batch_size:
                print("we don't have enough pool data")
                break;
                ##### apply model to pool pairs
            f, cov = model.predict_f(out_feats=train_feats,full_cov=True)

            IG = compute_pairwise_scores(f, cov)
            pool_pairs = bald_acquire(train_pairs, pool_pairs, batch_size, IG, train_idxs)
        ...
        if OPTIONS['task4']:
            ...
            dev_perc = 0
            test_perc = 0
            train_split, train_idxs, dev_idxs, test_idxs = split_data(pairs, idx_instance_list, train_perc, dev_perc,
                                                                      test_perc, cut=cut, cut_mode=cut_mode)
            logging.warn(
                'Training split is 100%, using complete set as test set (emulating data that is available to BWS)...')
            dev_idxs = train_idxs
        else:
            train_perc = 0.6
            dev_perc = 0.2
...
param_grid['add_features'] = list(
    chain.from_iterable(combinations(features, r) for r in [0, len(features) - 1, len(features)]))
param_grid['task4'] = [False]
param_grid['active'] = [False]
# run grid of options
if __name__ == '__main__':
    # seeds = []
    # for i in range(10):
    #     seeds.append([random.randint(1, 10000), random.randint(1, 10000)])
    # seeds = []
    # for i in range(10):
    #     seeds.append([random.randint(1, 10000), random.randint(1, 10000)])
    seeds = [[2779, 1356],
    ...
    for rnd_seed, np_seed in seeds:
        ...
        ###################################################
        elif sys.argv[3] == 'active':
            param_grid['active'] = [True]
        ###################################################

        experiment_dir = 'experiment_{}_{}_mybald_500_10_1000'.format(task, datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
        ...
        if param_grid['cut'] == [None]:
            ...
            param_grid['cut_mode'] = ['random_instances']
            if task == 'metaphor':
        ...
        with open(NGRAMS_PATH.format(task, n), 'r', encoding='utf-8') as f:
