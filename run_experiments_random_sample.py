...
def split_data(pairs, idx_instance_list, train_size=0.6, dev_size=0.2, test_size=0.2, cut=None, cut_mode=None):
    ...
    if cut is not None:
        ...
        with open(os.path.join(OPTIONS['experiment_dir'], 'items_{}_{}.txt'.format(str(cut), cut_mode)), 'w',encoding='utf-8') as f:
            ...
            for pair in cut_train_pairs:
                ...
                writer.writerow(row)
        with open(os.path.join(OPTIONS['experiment_dir'], 'items_instances_{}_{}.txt'.format(str(cut), cut_mode)), 'w',encoding='utf-8') as f:
...
def run(cut, cut_mode):
    ...
    if OPTIONS['active']:
        ...
        while True:
            print("\n\n\nactive learning start: current size = %d\nspearman_correlation = %f"%(len(train_pairs), spearman_correlation))
            # train model
            ...
            random_acquire(train_pairs,pool_pairs,batch_size)
...
if __name__ == '__main__':
    ...
    ##################################################
        ...
        experiment_dir = 'experiment_{}_{}_random_sample_500_10_1000'.format(task, datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
